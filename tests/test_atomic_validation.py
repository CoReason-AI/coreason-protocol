...
def create_protocol_with_terms(terms: list[str]) -> ProtocolDefinition:
    ...
    blocks = {
        ...
        "I": PicoBlock(block_type="I", description="Int", terms=[
            OntologyTerm(id="i1", label="Valid", vocab_source="MeSH", code="D1", origin=TermOrigin.USER_INPUT)
        ]),
        "O": PicoBlock(block_type="O", description="Out", terms=[
            OntologyTerm(id="o1", label="Valid", vocab_source="MeSH", code="D2", origin=TermOrigin.USER_INPUT)
        ]),
    }
...
def test_protocol_request_exists():
...
def test_invalid_term_semicolon():
...
def test_valid_term_embedded_and():
