...
async def test_service_async_lock(protocol_definition: ProtocolDefinition) -> None:
    ...
    async with ProtocolServiceAsync(client=mock_client) as svc:
        result = await svc.lock_protocol(protocol_definition, "user-async")
    ...
    assert result.approval_history.approver_id == "user-async"


@pytest.mark.asyncio  # type: ignore[misc]
async def test_service_async_lock_validation_error(protocol_definition: ProtocolDefinition) -> None:
    # Invalid protocol
...
def test_service_sync_lock(protocol_definition: ProtocolDefinition) -> None:
    ...
    with ProtocolService(client=mock_client) as svc:
        result = svc.lock_protocol(protocol_definition, "user-sync")
...
def test_service_sync_multiple_calls(protocol_definition: ProtocolDefinition) -> None:
    ...
    with ProtocolService(client=mock_client) as svc:
        # First call
        svc.lock_protocol(protocol_definition, "user-1")
        ...
        svc.lock_protocol(protocol_definition, "user-2")
    ...
    assert protocol_definition.approval_history.approver_id == "user-2"
...
@pytest.mark.asyncio  # type: ignore[misc]
async def test_service_async_compile(protocol_definition: ProtocolDefinition) -> None:
    async with ProtocolServiceAsync() as svc:
        strategies = await svc.compile_protocol(protocol_definition, "PUBMED")
...
def test_service_sync_compile(protocol_definition: ProtocolDefinition) -> None:
    with ProtocolService() as svc:
        strategies = svc.compile_protocol(protocol_definition, "PUBMED")
